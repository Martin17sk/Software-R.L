...
public interface ArticuloRepository extends JpaRepository<Articulo, String> {
    ...
    WHERE lp.id = :listaPrecioId
      AND a.codigo = :codigo
    """)
    Optional<ArticuloConPrecioActualDTO> buscarConPrecioPorCodigo(
            @Param("codigo") String codigo,
            @Param("listaPrecioId") Integer listaPrecioId
    );

    @Query("SELECT a.nombre FROM Articulo a ORDER BY a.nombre ASC")
    Page<String> findNombres(Pageable pageable);

    @Query("""
   SELECT new cl.cecinasllanquihue.gestor_precios.dto.ArticuloOptionDTO(a.codigo, a.nombre)
   FROM Articulo a
   ORDER BY a.nombre ASC
    """)
    List<ArticuloOptionDTO> findAllOptions();

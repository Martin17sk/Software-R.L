...
public class ListaPrecioController {
    ...
    public ResponseEntity<List<ListaPrecioDTO>> obtenerPorSistema(
            @PathVariable Integer sistemaId
    ) {
        return ResponseEntity.ok(
                listaPrecioService.obtenerListasPorSistema(sistemaId)
        );
    }

    @PostMapping
    public ResponseEntity<ListaPrecioDTO> crear(@RequestBody ListaPrecioDTO dto) {
        return ResponseEntity.ok(listaPrecioService.crearLista(dto));
    }
}
